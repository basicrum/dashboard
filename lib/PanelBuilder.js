...
class PanelBuilder {
    ...
    build(source, variables, options) {
        ...
        let result = this.buildElement(sourceTemplateDir, variables);
    ...
    buildElement(sourceTemplateDir, variables) {
        const element = this.loadJSON(this.panelTemplatePath, variables);
        const elementMerge = this.loadElementMerge(sourceTemplateDir, variables);
        return merge(element, elementMerge);
    }

    loadElementMerge(sourceTemplateDir, variables) {
        const file = this.buildFilePath(sourceTemplateDir, 'element.merge.json');
        const result = this.loadJSONIfExist(file, variables);
        if (!this.isEmptyObject(result)) {
            return result;
        }
        return this.loadJSONIfExist(this.buildFilePath(this.defaultSourceDir, 'element.merge.json'), variables);
    }
    ...
    loadJSONIfExist(sourceFile, variables) {
        if (fs.existsSync(sourceFile)) {
            return this.loadJSON(sourceFile, variables);
        }
        return {};
    }
    ...
    loadJSON(path, variables) {
        return JSON.parse(this.loadFile(path, variables));
    }

    loadFile(path, variables) {
        let result = fs.readFileSync(path, { encoding: 'utf8', flag: 'r' });
