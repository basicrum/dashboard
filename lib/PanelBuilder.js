...
class PanelBuilder {
    ...
    updateDataSource(result, sourceTemplateDir, variables, options) {
        ...
        targets["table"] = options["table"];
        targets["datasource"]["uid"] = options["datasourceUid"];
        result["datasource"]["uid"] = options["datasourceUid"];
        let queryTemplate = this.loadSourceFile(sourceTemplateDir, 'query.sql');
        targets["query"] = this.replaceVariables(queryTemplate, variables);
    }
